...
describe("resolveSessionDisplayName", () => {
  ...
  it("parses direct chat key with channel", () => {
    expect(resolveSessionDisplayName("agent:main:telegram:direct:+19257864429")).toBe(
      "Telegram · +19257864429",
    );
  });
  ...
  it("does not prefix non-typed sessions with labels", () => {
    expect(
      resolveSessionDisplayName(
        "agent:main:telegram:direct:+19257864429",
        row({ key: "agent:main:telegram:direct:+19257864429", label: "Tyler" }),
      ),
    ).toBe("Tyler");
  });
});
