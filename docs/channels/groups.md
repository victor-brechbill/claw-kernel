...
OpenClaw treats group chats consistently across surfaces: WhatsApp, Telegram, Discord, Slack, Signal, Microsoft Teams.
...
- WhatsApp/Telegram/Signal: use `groupAllowFrom` (fallback: explicit `allowFrom`).
...
When `channels.whatsapp.groups` or `channels.telegram.groups` is configured, the keys act as a group allowlist. Use `"*"` to allow all groups while still setting default mention behavior.
