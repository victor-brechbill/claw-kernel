...
describe("deliverOutboundPayloads", () => {
  ...
  // Signal tests removed - plugin deleted in NOVA-336
  ...
  it("preserves fenced blocks for markdown chunkers in newline mode", async () => {
    ...
    const cfg: OpenClawConfig = {
      channels: { "test-ext": { textChunkLimit: 4000, chunkMode: "newline" } },
    };
    const text = "```js\nconst a = 1;\nconst b = 2;\n```\nAfter";
  ...
  it("emits message_sent success for sendPayload deliveries", async () => {
    hookMocks.runner.hasHooks.mockImplementation((name: string) => name === "message_sent");
    const sendPayload = vi.fn().mockResolvedValue({ channel: "test-ext", messageId: "mx-1" });
    ...
    await vi.waitFor(() => {
      expect(hookMocks.runner.runMessageSent).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ channelId: "test-ext" }),
      );
    });
  });
