...
describe("redactConfigSnapshot", () => {
  ...
  it("redacts botToken in channel configs", () => {
    const snapshot = makeSnapshot({
      channels: {
        ...
        discord: { botToken: "fake-discord-bot-token-placeholder-value" },
      },
    });
    const result = redactConfigSnapshot(snapshot);
  ...
  it("redacts signingSecret fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        discord: { signingSecret: "discord-signing-secret-value-1234" },
      },
    });
    const result = redactConfigSnapshot(snapshot);
    const channels = result.config.channels as Record<string, Record<string, string>>;
    expect(channels.discord.signingSecret).toBe(REDACTED_SENTINEL);
  });
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        mychannel: {
          passwordFile: "/etc/openclaw/channel-password.txt",
          service: {
            passwordFile: "/etc/openclaw/service-password.txt",
            password: "super-secret-service-password",
          },
        },
      },
    });
    ...
    const ch = channels.mychannel;
    const service = ch.service as Record<string, unknown>;

    expect(ch.passwordFile).toBe("/etc/openclaw/channel-password.txt");
    expect(service.passwordFile).toBe("/etc/openclaw/service-password.txt");
    expect(service.password).toBe(REDACTED_SENTINEL);
  });
  ...
  it("handles deeply nested tokens in accounts", () => {
    const snapshot = makeSnapshot({
      channels: {
        discord: {
          accounts: {
            server1: { botToken: "fake-server1-token-abcdefghij" },
            server2: { appToken: "fake-server2-token-abcdefghij" },
          },
        },
      },
    });
    ...
    >;
    expect(channels.discord.accounts.server1.botToken).toBe(REDACTED_SENTINEL);
    expect(channels.discord.accounts.server2.appToken).toBe(REDACTED_SENTINEL);
  });
  ...
  it("still redacts 'token' (singular) fields", () => {
    const snapshot = makeSnapshot({
      channels: { discord: { token: "secret-discord-token-value-here" } },
    });
    const result = redactConfigSnapshot(snapshot);
    const channels = result.config.channels as Record<string, Record<string, string>>;
    expect(channels.discord.token).toBe(REDACTED_SENTINEL);
  });
  ...
  it("uses wildcard hints for array items", () => {
    const hints: ConfigUiHints = {
      "channels.discord.accounts[].botToken": { sensitive: true },
    };
    const snapshot = makeSnapshot({
      channels: {
        discord: {
    ...
    >;
    expect(channels.discord.accounts[0].botToken).toBe(REDACTED_SENTINEL);
    expect(channels.discord.accounts[1].botToken).toBe(REDACTED_SENTINEL);
  });
});
...
describe("restoreRedactedValues", () => {
  ...
  it("handles deeply nested sentinel restoration", () => {
    ...
    const result = restoreRedactedValues(incoming, original) as typeof incoming;
    expect(result.channels.discord.accounts.ws1.botToken).toBe("original-ws1-token-value");
    expect(result.channels.discord.accounts.ws2.botToken).toBe("user-typed-new-token-value");
  });
  ...
  it("round-trips config through redact → restore", () => {
    const originalConfig = {
      ...
      channels: {
        discord: { botToken: "fake-discord-token-placeholder-value" },
        telegram: {
  ...
  it("restores array items using wildcard uiHints", () => {
    ...
    const result = restoreRedactedValues(incoming, original, hints) as typeof incoming;
    expect(result.channels.discord.accounts[0].botToken).toBe("original-token-first-account");
    expect(result.channels.discord.accounts[1].botToken).toBe("user-provided-new-token-value");
  });
});
