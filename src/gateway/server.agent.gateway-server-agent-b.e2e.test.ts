...
describe("gateway server agent", () => {
  ...
  test("agent dedupe survives reconnect", { timeout: 60_000 }, async () => {
    await withGatewayServer(async ({ port: serverPort }) => {
      const dial = async () => {
        const conn = new WebSocket(`ws://127.0.0.1:${serverPort}`);
        await new Promise<void>((resolve) => conn.once("open", resolve));
        await connectOk(conn);
        return conn;
      };
