...
describe("resolveRequestedLoginProviderOrThrow", () => {
  ...
  it("throws when requested provider is not loaded", () => {
    const providers = [makeProvider({ id: "google-gemini-cli" }), makeProvider({ id: "minimax" })];

    expect(() =>
      resolveRequestedLoginProviderOrThrow(providers, "google-antigravity"),
    ).toThrowError(
      'Unknown provider "google-antigravity". Loaded providers: google-gemini-cli, minimax. Verify plugins via `openclaw plugins list --json`.',
    );
  });
});
